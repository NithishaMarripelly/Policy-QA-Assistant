...
st.markdown("""
    <style>
    /* Professional dark blue styling for the citation boxes */
    [data-testid="stInfo"] {
    ...
    /* Force a clean, small font on citation elements */
    [data-testid="stInfo"] * {
...
# --- 4. MAIN UI SETUP (Matches your screenshots) ---
    ...
    if st.button("Upload & Sync"):
        if uploaded_file:
            ...
            try:
                ...
                s3_client.put_object(Bucket=S3_BUCKET, Key=metadata_path, 
                ...
                with st.spinner("🔄 Assistant is reading the document..."):
                    job = bedrock_agent.start_ingestion_job(knowledgeBaseId=KNOWLEDGE_BASE_ID, dataSourceId=DATA_SOURCE_ID)
                    job_id = job['ingestionJob']['ingestionJobId']
                    while True:
                        status = bedrock_agent.get_ingestion_job(knowledgeBaseId=KNOWLEDGE_BASE_ID, dataSourceId=DATA_SOURCE_ID, ingestionJobId=job_id)['ingestionJob']['status']
                        if status == 'COMPLETE':
...
if "messages" not in st.session_state:
    st.session_state.messages = [{"role": "assistant", "content": "Hello! I'm your Policy Assistant. Once you've uploaded a document in the sidebar, feel free to ask me anything about it!"}]
